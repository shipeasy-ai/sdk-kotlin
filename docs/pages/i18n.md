...
helper (there is no `t()` / label-render API on `Engine` or `Client`).
Translation rendering happens in the **browser**, via the Shipeasy client SDK's
`t()` and a loader script.

What this SDK *does* provide is the **SSR wiring** so the browser SDK has the
right profile and the same bucketing on first paint:

- `Engine.i18nScriptTag(clientKey, profile)` — emits the i18n **loader**
  `<script>` tag. The loader fetches translations for the profile using the
  **public client key** (safe to embed in HTML).
- `Engine.bootstrapScriptTag(user, ..., i18nProfile)` — the bootstrap tag also
  carries the i18n profile in `data-i18n-profile`.
