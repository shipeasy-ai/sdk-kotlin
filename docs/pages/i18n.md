...
- `i18nScriptTag(clientKey?, profile?, baseUrl?)` — emits the i18n **loader**
  `<script>` tag. The loader fetches translations for the profile using the
  **public client key** (safe to embed in HTML). **Every argument is optional**:
  each falls back to what `configure()` set.
- `bootstrapScriptTag(user, ..., i18nProfile)` — the bootstrap tag also carries
  the i18n profile in `data-i18n-profile` (same default).
...
1. **Server (this SDK):** render the loader tag into your document `<head>`,
   ...
   // configure(clientKey = ..., profile = "{{PROFILE}}") already set both.
